...
if __name__=='__main__':
    ...
    parser.add_argument('--QFeatureFlag', type=str2bool, default=True, help='do activation quantization')
    ...
    parser.add_argument('--distill', type=str, default=None, choices=['kd', 'fd', 'crdst','hint', 'attention', 'similarity', 'correlation', 'vid', 'crd', 'kdsvd', 'fsp', 'rkd', 'pkt', 'abound', 'factor', 'nst'])
    parser.add_argument('--teacher_path', type=str, default='./results/CIFAR10_ResNet20/Qfeature_4bits_STE/checkpoint/best_checkpoint.pth', help='path for pretrained teacher model with quantizer')
    parser.add_argument('--teacher_arch', type=str, default='resnet20_fp', help='teacher model architecture')
    parser.add_argument('--kd_T', type=float, default=4, help='temperature for KD distillation')
    parser.add_argument('--kd_alpha', type=float, default=None, help='weight for classification')
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.distill:
                ...
                if args.distill in ['abound']:
                    preact = True
                feat_s, block_out_s, logit_s, fd_map_s = model_s(images, save_dict, lambda_dict, is_feat=True, preact=preact, flatGroupOut=flatGroupOut)
                with torch.no_grad():
            ...
            if args.distill:
                ...
                if args.distill == "crdst":
                    ...
                    loss_fd = criterion_kd(fd_map_s, fd_map_t) # feature distillation
                    loss_total = args.kd_beta * loss_div + args.kd_gamma * loss_fd # QFD Loss
                    ...
                    content = f"total_iter={total_iter}, loss_cls={loss_cls_value}, loss_fd={loss_fd_value}, loss_total={loss_total_value}"
